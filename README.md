...
现在记账软件要么逼你手动选分类，要么塞满广告，要么基础功能都放付费墙后面。安装包动辄几百 MB，开个 app 先看 5 秒开屏广告。

这个不一样。APK **3.9 MB**，首次加载后每次数据刷新只走 **55KB 流量**。没有广告、没有订阅、不需要注册、不碰你的隐私。

怎么用？跟 AI 说一声就行——「午饭花了 23 块 4」自动归类餐饮，「工资到账 8000」自动归类工资。Agent 替你做完分类、写表、同步所有脏活，App 端把数据渲染成饼图、柱状图、趋势线。数据存在你自己名下的飞书里，零服务器。
